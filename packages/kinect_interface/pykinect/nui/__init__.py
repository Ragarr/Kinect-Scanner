...
class ImageStream(object):
    ...
    def open(self, image_stream_type = 0, frame_limit = 2, resolution = ImageResolution.Resolution320x240, image_type = ImageType.Depth):
...
class SkeletonEngine(object):    
    ...
    def depth_image_to_skeleton(fDepthX, fDepthY, usDepthValue):
        ...
        fSkeletonZ = old_div((usDepthValue >> 3), 1000.0)
    ...
    def skeleton_to_depth_image(vPoint, scaleX = 1, scaleY = 1):
        ...
        if vPoint.z > _FLT_EPSILON: 
           ...
           pfDepthX = 0.5 + vPoint.x * ( old_div(_NUI_CAMERA_SKELETON_TO_DEPTH_IMAGE_MULTIPLIER_320x240, vPoint.z) ) / 320.0
           pfDepthY = 0.5 - vPoint.y * ( old_div(_NUI_CAMERA_SKELETON_TO_DEPTH_IMAGE_MULTIPLIER_320x240, vPoint.z) ) / 240.0
